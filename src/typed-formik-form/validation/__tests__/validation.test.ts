...
describe('validation', () => {
    ...
    describe('isValidArrayWithItems', () => {
        it('rejects if value is not an array', () => {
            expect(isArrayWithItems(undefined)).toBeFalsy();
            expect(isArrayWithItems(null)).toBeFalsy();
            expect(isArrayWithItems({})).toBeFalsy();
            expect(isArrayWithItems(2)).toBeFalsy();
            expect(isArrayWithItems('213')).toBeFalsy();
        });
        it('rejects if value is an empty array', () => {
            expect(isArrayWithItems([])).toBeFalsy();
        });
        it('accepts if value is an array with items > 0', () => {
            expect(isArrayWithItems([1])).toBeTruthy();
            expect(isArrayWithItems([1, 2])).toBeTruthy();
        });
    });
