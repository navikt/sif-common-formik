...
export const isFieldWithValue = (v: any): boolean => v !== '' && v !== undefined && v !== null;

export const isValidArrayWithItems = (v: any): boolean => {
    return Array.isArray(v) && v.length > 0;
};

export const isValidDatePickerDateString = (value: any): boolean => {
    return datepickerUtils.getDateFromDateString(value) !== undefined;
};

export const isAnswerdYesOrNo = (value): boolean => {
...
export const isNumberWithinRange = (value: number, range: { min?: number; max?: number }): boolean => {
    const { min, max } = range;
